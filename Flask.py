...
def main():
    if request.method == 'POST': # post 방식일때
        ...
        # 업로드된 파일 모델 분류후 file_topic 변수에 저장
        file_topic = lda_model.classification(file_path)
        # 업로드된 파일 목록 가져오기
        file_list = os.listdir(file_path)
        # db에 데이터 업데이트
        file.db_update(user_seq, file_list, file_topic)
        # 분류할 폴더별로 파일 이동
        file.replace_file(file_path, file_list, file_topic)
...
def drive():
    ...
    # 파일 크기 단위 변환하기
    convert_file_size = file.convert_size(sum_file_size)
    print('File Size:', convert_file_size, 'bytes') # 파일 크기 확인
    try:
        upload_time_list = os.listdir(file_path) # 업로드 폴더 가져오기
